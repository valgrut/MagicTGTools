...
def colorize_svg(svg_bytes: bytes, hex_color: str) -> bytes:
    """
    Colorize a set symbol
    """
    # Most Scryfall set SVGs use currentColor. Make it to use a fill color
    txt = svg_bytes.decode("utf-8")
    if "currentColor" in txt:
        txt = txt.replace("currentColor", hex_color)
        
    # Inject a 'fill=' if none present into a tag
    if "fill=" not in txt:
        txt = txt.replace("<svg", f"<svg fill='{hex_color}'", 1)
